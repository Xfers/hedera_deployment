...
func CreateAdminKeyListDeployTestnetXsgd() hedera.Key {
...
func CreateSupplyKeyListDeployTestnetXsgd() hedera.Key {
...
func CreateBlacklisterKeyListDeployTestnetXsgd() hedera.Key {
...
func CreateKycKeyListDeployTestnetXsgd() hedera.Key {

	keys := make([]hedera.PublicKey, 2)

	keys[0], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_KYC_PUBLIC_KEY_1"))
	keys[1], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_KYC_PUBLIC_KEY_2"))
...
func CreateFeeKeyListDeployTestnetXsgd() hedera.Key {

	keys := make([]hedera.PublicKey, 3)

	keys[0], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_FEE_PUBLIC_KEY_1"))
	keys[1], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_FEE_PUBLIC_KEY_2"))
	keys[2], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_FEE_PUBLIC_KEY_3"))
...
func CreateWiperKeyListDeployTestnetXsgd() hedera.Key {

	keys := make([]hedera.PublicKey, 2)

	keys[0], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_WIPER_PUBLIC_KEY_1"))
	keys[1], _ = hedera.PublicKeyFromString(os.Getenv("HEDERA_XSGD_TESTNET_WIPER_PUBLIC_KEY_2"))
...
func create_token_tx_hedera_testnet_xsgd() {
	...
	tokenCreateTransaction, err := hedera.NewTokenCreateTransaction().
		...
		SetTokenName("XSGD-testnet-Oct12").                        // token name
		SetTokenSymbol("XSGD-testnet-Oct12").                      // token symbol
		SetTreasuryAccountID(treasuryAccountId).                   // treasury account id
		SetAdminKey(CreateAdminKeyListDeployTestnetXsgd()).        // owner
		SetSupplyKey(CreateSupplyKeyListDeployTestnetXsgd()).      // minter/burner
		SetFreezeKey(CreateBlacklisterKeyListDeployTestnetXsgd()). // blacklister
		//SetWipeKey(CreateWiperKeyListDeployTestnetXsgd()).         // wiper
		//SetKycKey(CreateKycKeyListDeployTestnetXsgd()).            // KYC
		//SetFeeScheduleKey(CreateFeeKeyListDeployTestnetXsgd()).    // Fee
...
func main() {
	create_token_tx_hedera_testnet_xsgd()
}
