...
func mint_hedera_testnet_xidr() {
	...
	token_id, err := hedera.TokenIDFromString("0.0.47996111")
	tokenMintTransaction, err := hedera.NewTokenMintTransaction().
		SetTransactionID(transactionId).
		SetTokenID(token_id).
		SetAmount(200000000). // 200 mil = 200 token
		FreezeWith(client)
	...
	//txResponse, err := tokenMintTransaction.Sign(supplierPrivateKey1).Sign(supplierPrivateKey2).Sign(supplierPrivateKey3).Execute(client)
